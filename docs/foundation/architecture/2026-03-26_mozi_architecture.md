...
│   │                 │  │                     │  │                     │ │
│   │  LLM Context    │  │  向量库 (Qdrant)    │  │  文件系统           │ │
│   │  (内存/LRU)     │  │  本地Qdrant/嵌入式   │  │  SQLite             │ │
│   │                 │  │  Milvus/云端服务     │  │                     │ │
│   │  当前会话上下文  │  │  语义向量/记忆索引   │  │  永久持久化知识      │ │
│   │  Agent运行时状态 │  │  会话级数据          │  │  用户偏好/项目知识    │ │
│   │  即时思考(CoT)  │  │  RAG检索             │  │  跨会话记忆          │ │
│   └─────────────────┘  └─────────────────────┘  └─────────────────────┘ │
│                                                                         │
...
| 层级 | 载体 | 数据类型 | 持久性 | 访问延迟 | 类比人类记忆 |
|------|------|----------|--------|----------|--------------|
| 工作记忆 | LLM Context (内存) | 当前会话上下文、Agent状态、CoT | 会话级 | 极低 | 短期工作记忆 |
| 短期记忆 | 向量库 (Qdrant) + SQLite | 语义向量、代码嵌入、会话记录 | 会话级持久化 | 低 | 短期记忆 |
| 长期记忆 | 文件系统 + SQLite | 用户偏好、项目知识、历史经验 | 永久 | 中 | 长期记忆 |
...
**短期记忆策略 - 摘要压缩**：
- 当对话过长时，调用 LLM 将旧对话总结成一段话
- 释放上下文空间，保留关键信息

**长期记忆策略 - RAG 检索**：
- 根据当前意图，从向量库中检索最相关的记忆片段
- 注入 Prompt 供 Agent 使用
...
- 工作→短期：会话结束或超过时间阈值
- 短期→长期：访问频率降低或会话归档时
- 长期记忆召回：通过 `memory:recall` 工具按需加载
...
| **记忆系统** | 三层记忆+自动分层+RAG检索 | 云端Knowledge Base | 插件依赖 | 本地Markdown |
...
3. **三层记忆+自动分层+RAG**: 竞品多为单一存储或需手动配置
