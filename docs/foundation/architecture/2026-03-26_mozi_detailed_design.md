...
| 维度 | 隔离级别 | 说明 |
|------|----------|------|
| 工具权限 | 子集继承 | 子会话权限 ≤ 父会话权限 |
| 内存状态 | 完全隔离 | 独立 LLM 上下文 |
| 文件系统 | 共享（只读） | 共享项目目录 |
| 会话状态 | 共享 | 向量记忆索引共享 |
...
│                   （LLM Context，内存）                  │
│                                                         │
│  当前会话上下文、Agent 运行时状态、即时思考过程 (CoT)      │
│                                                         │
│  生命周期：会话级，使用滑动窗口保留最近对话                 │
└─────────────────────────┬───────────────────────────────┘
                          │ 会话结束 / 滑动窗口触发
┌─────────────────────────▼───────────────────────────────┐
│                        短期记忆                           │
...
│                                                         │
│  语义向量、代码嵌入、记忆索引、会话摘要                    │
│  RAG 检索：按需召回相关记忆片段注入 Prompt                 │
│                                                         │
│  生命周期：会话级持久化，归档后转入长期记忆                 │
└─────────────────────────┬───────────────────────────────┘
                          │ 会话归档 / 重要性评分触发
┌─────────────────────────▼───────────────────────────────┐
│                        长期记忆                           │
│                   （文件系统 + SQLite）                   │
│                                                         │
│  用户偏好（如"喜欢用 Python"）、项目知识、历史经验教训      │
│  通过 memory:recall 工具按需召回                          │
│                                                         │
│  生命周期：永久持久化，跨会话共享                         │
└─────────────────────────────────────────────────────────┘
```
...
| 流转方向 | 触发条件 | 可配置 | 异常处理 |
|----------|----------|--------|----------|
| 工作→短期 | 会话结束（正常）或 超时30分钟无访问 | 是 | 进程崩溃时由 checkpoint 恢复 |
| 短期→长期 | 访问频率 < N次/30天（N=5，可配置）或会话归档 | 是 | 软删除，30天内可恢复 |
| 长期召回 | Agent 通过 `memory:recall` 工具主动检索 | - | 向量库查询，返回相似记忆 |

**记忆写入策略 - 重要性评分**：
- 让 LLM 给当前对话打分（1-10 分）
- 例如：用户说"你好"（1 分，不记）；用户说"我习惯用 Tab 缩进"（10 分，存入长期记忆）

**记忆召回策略 - 混合检索**：
- 关键词检索 (BM25)：找精确术语
- 语义检索 (Embedding)：找相似意图
- 时间衰减：检索分数 = 语义相似度 × 时间衰减系数
...
1. **静态分析为主**（第一阶段）
   - 基于 import/require 图分析文件依赖
   - 基于配置文件（如 package.json, requirements.txt）分析显式依赖
   - 基于代码模式（如 API 调用链）推断隐式依赖

2. **完整性判定**
   - 依赖覆盖率达到 80% 以上 → 认为"完整"，允许 COMPLEX 路由
   - 依赖覆盖率 < 80% → 认为"不完整"，触发降级逻辑

3. **降级策略**（不完整时的处理）
   - 强制降级为 MEDIUM 路由
   - 向用户展示推断出的依赖图，提示可能遗漏的依赖
   - 提供手动补充依赖的接口（作为临时 workaround）
...
**memory:recall 工具签名**：
- 参数：`path: str`（MEMORY.md 文件路径）
- 返回：完整的 MEMORY.md 内容
- 触发场景：上下文压缩后，Agent 需要回溯历史决策时
...
**与上下文压缩的集成**：
1. 压缩前 → 执行 MEMORY.md 冲刷
2. 压缩中 → 保留 MEMORY.md 锚点，卸载详细内容
3. 压缩后 → Agent 可通过 memory:recall 按需召回
...
**难点描述**：数据在三层记忆间流转时需要保证：
- 流转触发条件准确
- 数据格式正确转换（如会话摘要 → 长期记忆向量）
- 流转原子性（失败回滚）
...
```
target_tokens = model_window_size * 0.8 - safety_margin
actual_tokens = current_context_token_count
remaining = model_window_size - actual_tokens

if remaining < safety_margin:
    # 紧急压缩
    compression_ratio = 0.5
elif remaining < target_tokens:
    # 正常压缩
...
**参数说明**：
- model_window_size：模型上下文窗口大小
- safety_margin：安全边界（建议 2000 tokens）
...
| 维度 | 策略 |
|------|------|
| 创建时机 | 工具调用完成后、Agent 步骤完成后、会话结束前 |
| 创建原子性 | 使用写入日志（WAL）后写 checkpoint |
| 存储位置 | 工作记忆（内存）+ 短期记忆（SQLite）双写 |
| 保留数量 | 最近 10 个 |
| 保留期限 | 7 天 |
| 回滚选择 | 支持选择任意 checkpoint 回滚 |
| 恢复粒度 | 完整恢复或选择性恢复（仅状态/仅上下文） |
...
│   │   │   ├── __init__.py
│   │   │   ├── manager.py        # 记忆管理器
│   │   │   ├── working.py        # 工作记忆（LLM Context）
│   │   │   ├── short_term.py     # 短期记忆（向量库 + SQLite）
│   │   │   ├── long_term.py     # 长期记忆（文件系统）
...
│  │(Context)│◄─┤(VectorDB/ │◄─┤   (Filesystem/SQLite)   │  │
│  │ Memory  │  │  SQLite)  │  │                          │  │
│  └─────────┘  └───────────┘  └──────────────────────────┘  │
│                                                              │
│  ┌─────────────┐  ┌─────────────────┐  ┌───────────────┐  │
...
class MemoryTier(ABC, Generic[T]):
    """记忆层抽象"""
...
class MemoryManager:
    ...
    def __init__(
        self,
        working: MemoryTier,      # 工作记忆（LLM Context）
        short_term: MemoryTier,   # 短期记忆（向量库 + SQLite）
        long_term: MemoryTier     # 长期记忆（文件系统）
    ):
        self.working = working        # 内存（LRU）
        self.short_term = short_term  # 向量库
        self.long_term = long_term    # 文件系统
    ...
    async def importance_score(content: str) -> float:
        """重要性评分：LLM 打分 1-10"""
        ...
```
...
**决策**：
- stdio 传输：连接池复用子进程（限制最大 5 个）
- HTTP/WebSocket 传输：连接池复用 HTTP 连接（限制最大 10 个）

**代价**：连接池管理增加复杂度，需要处理进程僵死检测和回收。
...
class EventBus:
    ...
    def subscribe(self, handler: Callable) -> Subscription:
        """多消费者订阅 - 需重构"""
        ...
```
...
本文档与 `architecture.md` 术语对照：

| 本文档术语 | architecture.md 术语 | 统一用法 |
|-----------|---------------------|---------|
| 编排层 | Orchestration Layer | 编排层 |
| 能力层 | Capabilities Layer | 能力层 |
| 接入层 | Ingress Layer | 接入层 |
| 基础设施层 | Infrastructure Layer | 基础设施层 |
| 工作/短期/长期记忆 | Working/Short-Term/Long-Term Memory | 三层记忆 |
