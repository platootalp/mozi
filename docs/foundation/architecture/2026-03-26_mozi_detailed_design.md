...
| 流转方向 | 触发条件 | 可配置 | 异常处理 |
|----------|----------|--------|----------|
| 热→温 | 会话结束（正常）或 超时30分钟无访问 | 是 | 进程崩溃时由 checkpoint 恢复 |
| 温→冷 | 访问频率 < N次/30天（N=5，可配置） | 是 | 向量库软删除，30天内可恢复 |
| 冷→归档 | 保留期限 > 180天（可配置） | 是 | 归档前生成索引，支持快速召回 |
