...
| Memory | Memory 是独立模块，负责跨会话的四类记忆（Episodic/Preference/Experience/Procedural）；Context 通过 MemoryManager.record() 和 MemoryManager.recall() 与 Memory 交互，不直接操作存储 |
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v1.3 | 2026-03-29 | 与 Memory 模块对齐：更新 Memory 边界描述（Memory 是独立模块）、移除重复的 Experience 类型（由 Memory 拥有）、移除重复的 USER_PREFERENCE/EXPERIENCE ContextType |
